...
cout << "depth queue size:" << queueSize << endl;
  ...
  void imageAverage()
  {
    ...
    cv::Mat addDepthInt_8U(DEPTHROWS, DEPTHCOLS, CV_8U, cv::Scalar(0)), addDepthIntCanny;
	...
	if(trueFrameCount == NUMOFDEPTH){dividePictureInt(addDepthInt);}//NUMOFDEPTHで割る
	...
	if(trueFrameCount >= NUMOFDEPTH){
	  ...
	  convert16Uto8U_ver2(addDepthInt, addDepthInt_8U);
	  cv::Canny(addDepthInt_8U, addDepthIntCanny, 200, 0);
	  cv::imshow("addDepthInt_8U", addDepthInt_8U);
