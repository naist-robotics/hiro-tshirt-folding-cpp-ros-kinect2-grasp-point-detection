...
static const int DEPTHROWS = 540;
static const int DEPTHCOLS = 960;
...
private:
  ...
  void imageAverage()//Mat.at<double>(0,0) = 1212;で特定の要素にコピーできる？　depth 960:540
  {
    ...
    const int sizes[] = {DEPTHROWS, DEPTHCOLS, NUMOFDEPTH};
    cv::Mat accumulateDepth(sizeof(sizes)/sizeof(int), sizes, CV_16U);//オリジナルのdepthはCV_16U
    //cout << sizeof(sizes) << "  " << sizeof(int) << "   " << sizeof(sizes)/sizeof(int) << endl; //結果は 12  4   3
    cv::Mat addDepth(DEPTHROWS, DEPTHCOLS, CV_32F, cv::Scalar(0));
    cv::Mat addDepthInt(DEPTHROWS, DEPTHCOLS, CV_16U, cv::Scalar(0));
    cv::Mat depth_median, addDepth_16, depthDisp_addDepth_16, addDepthDisp;
	...
	size_t pixelCount = 0, zeroPixelCount = 0;
	if(trueFrameCount <= NUMOFDEPTH){//NUMOFDEPTHの数だけ格納していく
	  cv::medianBlur(depth, depth_median, 3);//足す前にメディアンかける

	  cout << "channel:" << depth_median.channels() << "  step:" << depth_median.step
	       << "  rows:" << depth_median.rows << "  cols:" << depth_median.cols << endl;
	  //int depth_median_channels = depth_median.channels();
	  //int depth_median_step = depth_median.step;
	  //int depth_median_rows = depth_median.rows;
	  //int depth_median_cols = depth_median.cols;

	  //実際に足し合わせていくループ
	  //add2Pictures(addDepth, depth_median);
	  add2PicturesInt(addDepthInt, depth_median);

	  /*
	  for(int r = 0; r < depth_median_rows; r++){
	    for(int c = 0; c < depth_median_cols; c++){
	      for(int channel = 0; channel < depth_median_channels; channel++){
		pixelCount++;

		if(r == depth_median.rows/2 && c == depth_median.cols/2){
		  int centerDepth = depth_median.data[r*depth_median_step + c + depth_median_channels];
		  cout << "at center depth: " << depth_median.at<uchar>(r, c) << endl;
		  cout << "centerDepth:" << centerDepth << endl;
		  cout << "ADD center pixel value: " << addDepth.data[r*addDepth.step + c + addDepth.channels()] 
		       << "   center pixel value: " << depth_median.data[r*depth_median_step + c + depth_median_channels] << endl;
		}

		addDepth.data[r*DEPTHCOLS + c] = depth_median.data[r*DEPTHCOLS + c] / 65535;

		if(depth.data[r*DEPTHCOLS + c] == 0){zeroPixelCount++;}

		if(r == DEPTHROWS/2 && c == DEPTHCOLS/2){
		  cout << "ADD center pixel value: " << addDepth.data[r*DEPTHCOLS + c] 
		       << "   center pixel value: " << depth.data[r*DEPTHCOLS + c] << endl;
		}
	  ...
	  */
	  ...
	  for(int r = 0; r < DEPTHROWS; ++r){
	    //uint16_t *depthPtr = depth.ptr<uint16_t>(r);
	    //float_t *addDepthPtr = addDepth.ptr<float_t>(r);
	    for(int c = 0; c < DEPTHCOLS; ++c){
	      pixelCount++;
	      if(r == DEPTHROWS/2 && c == DEPTHCOLS/2){
		cout << "ADD center pixel value: " << addDepth.data[r*addDepth.step + c + addDepth.channels()] 
		     << "   center pixel value: " << depth_median.data[r*depth_median.step + c + depth_median.channels()] << endl;
	      }
	      addDepth.data[r*DEPTHCOLS + c] = depth_median.data[r*DEPTHCOLS + c] / 65535;
	      if(depth.data[r*DEPTHCOLS + c] == 0){zeroPixelCount++;}
	      if(r == DEPTHROWS/2 && c == DEPTHCOLS/2){
		cout << "ADD center pixel value: " << addDepth.data[r*DEPTHCOLS + c] 
	      ...
	      //if(depthPtr[c] == 0){zeroPixelCount++;}
	      //cout <<"depthPtr: " << *depthPtr << endl;
	    }
	  }
	  if(trueFrameCount == NUMOFDEPTH){
	    for(int r = 0; r < DEPTHROWS; ++r){
	      for(int c = 0; c < DEPTHCOLS; ++c){
		addDepth.data[r*DEPTHCOLS + c] = addDepth.data[r*DEPTHCOLS + c] / NUMOFDEPTH;
	      }
	    } 
	    dispDepth(addDepth, addDepthDisp, DEPTHMAX, DEPTHMIN);
	  }
	  */
	  cout << "pixel: " << pixelCount << "  zeroPixel: " << zeroPixelCount << endl;
	  /*
	  for(int i = 0; i < DEPTHROWS; i++){
	    for(int j = 0; j < DEPTHCOLS; j++){
	      accumulateDepth.at<double>(i, j, frameCount-1) = depth_median.at<double>(i, j);
	      //accumulateDepthじゃなくてもaddDepthに直接入れてもいいんじゃね？
	      //とりあえずaccumulateDepthからaddDepthに入れる方法でやる
	    }
	  }
	  if(trueFrameCount == NUMOFDEPTH){
	    for(int num = 0; num < NUMOFDEPTH; num++){
	      for(int i = 0; i < DEPTHROWS; i++){
		for(int j = 0; j < DEPTHCOLS; j++){
		  addDepth.at<double>(i, j) += accumulateDepth.at<double>(i, j, num);
		  if(i == 450 && j == 250){
		    cout << "num=" << num <<"i=450, j=250: " << accumulateDepth.at<double>(i, j, num) << endl;;
		  }
		}
	      }
	    }
	    for(int i = 0; i < DEPTHROWS; i++){
	      for(int j = 0; j < DEPTHCOLS; j++){
		addDepth.at<double>(i, j) = addDepth.at<double>(i, j) / NUMOFDEPTH;
	      }
	    }
	    addDepth.convertTo(addDepth_16, CV_16U);
	    dispDepth(addDepth_16, depthDisp_addDepth_16, DEPTHMAX, DEPTHMIN); 
	  }
	  */
	}
	if(trueFrameCount == NUMOFDEPTH){dividePictureInt(addDepthInt);}

        dispDepth(depth, depthDisp, 12000.0f);

	dispDepth(addDepthInt, addDepthDisp, DEPTHMAX, DEPTHMIN);

        combine(color, depthDisp, combined);//コメントアウトするとコアダンプ
        ///combined = color;

        cv::putText(combined, oss.str(), pos, font, sizeText, colorText, lineText, CV_AA);
        cv::imshow("Image Viewer", combined);//多分これで画像
	cv::imshow("depth median", depth_median);
	//if(trueFrameCount >= NUMOFDEPTH){cv::imshow("add Depth", addDepthDisp);}
	if(trueFrameCount >= NUMOFDEPTH){cv::imshow("add Depth", addDepthDisp);}

	//cv::imshow("average viewer", depthDisp_addDepth_16);
  ...
  void add2Pictures(cv::Mat &in1, const cv::Mat &in2){
    //in1がaddDepthで深度32F、in2がdepth_medianで深度16U
    //オリジナルのdepthはCV_16U
...
#pragma omp parallel for
    for(int r = 0; r < in1.rows; ++r){
      ...
      float *itOut = tmp.ptr<float>(r);

      for(int c = 0; c < in1.cols; ++c, ++it1In, ++it2In){
        *itOut = *it1In + (*it2In / 65535);
  ...
  void add2PicturesInt(cv::Mat &in1, const cv::Mat &in2){
    //in1がaddDepthで深度16U、in2がdepth_medianで深度16U
    //オリジナルのdepthはCV_16U
    //cv::Mat tmp = cv::Mat(in1.rows, in1.cols, CV_16U);//一旦tmpに入れて最後にoutに入れる

#pragma omp parallel for
    for(int r = 0; r < in1.rows; r++){
      uint16_t *it1In = in1.ptr<uint16_t>(r);
      const uint16_t *it2In = in2.ptr<uint16_t>(r);
  ...
  void dividePictureInt(cv::Mat &in1){
#pragma omp parallel for
    for(int r = 0; r < in1.rows; r++){
      uint16_t *it1In = in1.ptr<uint16_t>(r);
	...
	*it1In = *it1In / NUMOFDEPTH;
	if(r == in1.rows/2 && c == in1.rows/2){
          cout << "divided add depth:" << *it1In << endl;
	}
      }
    }
  }
  ...
  void average10Pictures(const cv::Mat &in0, const cv::Mat &in1, const cv::Mat &in2, const cv::Mat &in3, const cv::Mat &in4, const cv::Mat &in5,
			 const cv::Mat &in6, const cv::Mat &in7, const cv::Mat &in8, const cv::Mat &in9, cv::Mat &out){//画像in1とin2の平均を出力
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //オリジナルのdepthはCV_16U
