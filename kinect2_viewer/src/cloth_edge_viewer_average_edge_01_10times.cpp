...
static const int NUMOFDEPTH = 200;
static const int DEPTHROWS = 540;
static const int DEPTHCOLS = 960;

static const float DEPTHMAX = 7200.0f;//深度画像を見やすくする
...
cout << "callback started 02" << endl;

cout << "color queue size:" << queueSize << endl;
    readImage(imageColor, color);
	unsigned int colorint = color.depth();
	PrintDepth(colorint);
cout << "callback started 03" << endl;

cout << "depth queue size:" << queueSize << endl;
  ...
  void imageAverage()
  {
    ...
    cv::Mat addDepthInt;
    cv::Mat addDepth_8U(DEPTHROWS, DEPTHCOLS, CV_8U, cv::Scalar(0)), addDepthIntCanny;
	...
	if(trueFrameCount <= NUMOFDEPTH){//NUMOFDEPTHの数だけ格納していく
	  ...
	  add2Pictures(addDepth, depth_median);//実際に足し合わせていく
	}
	if(trueFrameCount == NUMOFDEPTH){
	  dividePicture(addDepth);//NUMOFDEPTHで割る
	  //convert32Sto16U(addDepth, addDepthInt);//
addDepth.convertTo(addDepthInt, CV_16U);
	}
	...
	if(trueFrameCount == NUMOFDEPTH){
	  ...
	  convert32Sto8U(addDepth, addDepth_8U);

	  cv::Canny(addDepth_8U, addDepthIntCanny, 180, 5);
	  //cv::imshow("addDepth_8U", addDepth_8U);
	  cv::imshow("Canny edge", addDepthIntCanny);
...
cout << "four" << endl;
  ...
  void add2Pictures(cv::Mat &in1, const cv::Mat &in2){
    //in1がaddDepthで深度32S、in2がdepth_medianで深度16U
    //オリジナルのdepthはCV_16U
...
#pragma omp parallel for
  ...
  void add2PicturesInt(cv::Mat &in1, const cv::Mat &in2){//in1にin2を足す 深度は16U
...
#pragma omp parallel for
	...
	if(*it1In > DEPTHMAX){
	  ...
	  *itOut = *it1In - DEPTHMIN + 10;//とりあえず値が範囲内で最小のものを10にする
	  //つまり画像の値としては 0, 10〜110(DEPTHMAX-DEPTHMIN), 255 になる
	  if(r == in1.rows/2 && c == in1.rows/2){
	    cout << "convert16Uto8U center pixel   in:" << *it1In << "  out:" << *itOut << endl;
	  }
	}
      }
    }
  }
  void convert16Uto8U_ver2(const cv::Mat &in1, cv::Mat &out){//16Uのin1を8UのoutにMAXVALUEとMINVALUEに基づいて変換
#pragma omp parallel for
    for(int r = 0; r < in1.rows; r++){
      const uint16_t *it1In = in1.ptr<uint16_t>(r);
      uint8_t *itOut = out.ptr<uint8_t>(r);
	...
	if(*it1In > DEPTHMAX){
	  ...
	  *itOut = (*it1In - DEPTHMIN)*2 + 10;//とりあえず値が範囲内で最小のものを10にする
	  //ついでに2倍にするので2きざみの値
	  //つまり画像の値としては 0, 10〜220(DEPTHMAX-DEPTHMIN), 255 になる
	  if(r == in1.rows/2 && c == in1.rows/2){
	    cout << "convert16Uto8U center pixel   in:" << *it1In << "  out:" << *itOut << endl;
	  }
	}
...
int output = 0;
  void convert32Sto8U(const cv::Mat &in1, cv::Mat &out){//16Uのin1を8UのoutにMAXVALUEとMINVALUEに基づいて変換
#pragma omp parallel for
    for(int r = 0; r < in1.rows; r++){
	...
	if(*it1In > DEPTHMAX){
	  ...
	  *itOut = (uint8_t)(*it1In - DEPTHMIN) * (255/(DEPTHMAX - DEPTHMIN));
	  if(r == in1.rows/2 && c == in1.rows/2){
	    output = (uint8_t)(*it1In - DEPTHMIN) * (255/(DEPTHMAX - DEPTHMIN));
	    cout << "convert32Fto8U center pixel   in:" << *it1In << "  out:" << *itOut << ", " << output << endl;
	  }
	}
      }
    }
