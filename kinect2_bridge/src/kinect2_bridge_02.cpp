...
static const float DEPTHMAX = 7800.0f;//深度画像を見やすくする
static const float DEPTHMIN = 7200.0f;

static const int DEPTHROWS = 424;
static const int DEPTHCOLS = 512;

static const char PATTERN[] = "006";
...
/*
    ...
    dispDepth(depth_median, addDepthDisp, DEPTHMAX, DEPTHMIN);

	line(addDepthDisp, Point(addDepthDisp.cols/2-100, 0), Point(addDepthDisp.cols/2-100, addDepthDisp.rows), Scalar(255,200,200), 1, 4);
	line(addDepthDisp, Point(addDepthDisp.cols/2+100, 0), Point(addDepthDisp.cols/2+100, addDepthDisp.rows), Scalar(255,200,200), 1, 4);
	line(addDepthDisp, Point(addDepthDisp.cols/2-200, 0), Point(addDepthDisp.cols/2-200, addDepthDisp.rows), Scalar(255,200,200), 1, 4);
	line(addDepthDisp, Point(addDepthDisp.cols/2+200, 0), Point(addDepthDisp.cols/2+200, addDepthDisp.rows), Scalar(255,200,200), 1, 4);


    cv::imshow("base depth", addDepthDisp);
*/
...
/*
    ...
    if(addCount <= NUMOFDEPTH){
      ...
      add2Pictures(addDepth, depth_median);//素のデータを1000倍して足していく
    }
    if(addCount == NUMOFDEPTH){
      dividePicture(addDepth);
      convert32Sto16U(addDepth, addDepth_16U);//入りきらないので100で割る
    }
	...
	if(addCount == NUMOFDEPTH){
	  ...
	  //cv::Canny(addDepth_8U, addDepth_Canny01, 100, 10);//180, 5 or 180, 10
	  cv::Canny(addDepth_8U, addDepth_Canny02, 150, 10);
	  //cv::Canny(addDepth_8U, addDepth_Canny03, 180, 10);
	  //cv::Canny(addDepth_8U, addDepth_Canny04, 200, 10);
	  //cv::Canny(addDepth_8U, addDepth_Canny05, 250, 10);
	  ...
	  cv::imshow("add gray Depth", addDepth_8U);

	  //char str1[100], str2[100], str3[100], str4[100], str5[100], strDepth[100];
	  char str2[100], strDepth[100];

	  //sprintf(str1, "clothEdge/edgeA%2d.png", PATTERN);
	  sprintf(str2, "clothEdgeDataSet/edge%s.png", PATTERN);
	  //sprintf(str3, "clothEdge/edgeC%2d.png", PATTERN);
	  //sprintf(str4, "clothEdge/edgeD%2d.png", PATTERN);
	  //sprintf(str5, "clothEdge/edgeE%2d.png", PATTERN);
	  sprintf(strDepth, "clothEdgeDataSet/depth%s.png", PATTERN);
	  //cv::imwrite(str1, addDepth_Canny01);
	  cv::imwrite(str2, addDepth_Canny02);
	  //cv::imwrite(str3, addDepth_Canny03);
	  //cv::imwrite(str4, addDepth_Canny04);
...
//inは16U
  ...
  void processIrDepth(const cv::Mat &ir, const cv::Mat &depth, std::vector<cv::Mat> &images, const std::vector<Status> &status, libfreenect2::Frame *depthFrame)
  {
    ...
    if(status[DEPTH_SD])
    {
      ...
      //depth.convertTo(images[DEPTH_SD], CV_32F, 1);////////////////////////////////////////////////////////////////////
    ...
    if(status[DEPTH_SD_RECT] || status[DEPTH_QHD] || status[DEPTH_HD])
    {
      ...
      //depth.convertTo(depthShifted, CV_32F, 1, depthShift);//分解能0.1mmにしたいので１０倍
